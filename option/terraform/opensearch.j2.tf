...
resource "oci_identity_policy" "starter_opensearch_policy" {
  ...
  statements = [
    "Allow service opensearch to manage vnics in compartment id ${local.lz_app_cmp_ocid}",
    "Allow service opensearch to use subnets in compartment id ${local.lz_app_cmp_ocid}",
    "Allow service opensearch to use network-security-groups in compartment id ${local.lz_app_cmp_ocid}",
    "Allow service opensearch to manage vcns in compartment id ${local.lz_app_cmp_ocid}",
  ]
}
...
locals {
  ...
  db_port = "9200"
  local_db_url = data.oci_opensearch_opensearch_cluster.starter_opensearch.opensearch_fqdn
  local_jdbc_url = format("jdbc:opensearch://https://%s:9200/?hostnameVerification=false", local.db_url)
