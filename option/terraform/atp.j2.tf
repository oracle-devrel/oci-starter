...
locals {
  ...
  tf_local_db_url = replace(data.oci_database_autonomous_database.starter_atp.connection_strings[0].profiles[local.index_profile].value, " ", "")
  tf_local_jdbc_url = format("jdbc:oracle:thin:@%s", local.db_url)
  # Create List of 'name' values from source objet list
  tf_local_ords_url = replace(data.oci_database_autonomous_database.starter_atp.connection_urls[0].apex_url, "ords/apex", "ords" )
}
