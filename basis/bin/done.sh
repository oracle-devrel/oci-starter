...
if [ ! -z "$UI_URL" ]; then
  ...
  if [ ! -z "$TEST_NAME" ]; then
    ...
    if [ "$TF_VAR_deploy_type" == "compute" ]; then
      ...
      scp -r -o StrictHostKeyChecking=no $BASTION_PROXY_COMMAND opc@$COMPUTE_IP:/home/opc/*.log target/.
      scp -r -o StrictHostKeyChecking=no $BASTION_PROXY_COMMAND opc@$COMPUTE_IP:/home/opc/app/*.log target/.
      if [ "$TF_VAR_language" == "java" ]; then
        if [ "$TF_VAR_java_framework" == "tomcat" ]; then
            ssh -o StrictHostKeyChecking=no $BASTION_PROXY_COMMAND opc@$COMPUTE_IP "sudo cp -r /opt/tomcat/logs /tmp/tomcat_logs; sudo chown -R opc /tmp/tomcat_logs"
            scp -r -o StrictHostKeyChecking=no $BASTION_PROXY_COMMAND opc@$COMPUTE_IP:/tmp/tomcat_logs target/.
        fi
      fi
    fi 
  fi
